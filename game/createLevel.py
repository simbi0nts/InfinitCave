...
class CreateFirstLevel():
    ...
    def tiltNewLevelFunc(self):
        self.tiltNewLevel = [[config.WALLS["WALL_ALONE"] for x in range(config.fullLevelMap_X + (config.X_CONST * 6))]
    ...
    def createItems(self):
        ...
        if self.levelMap != []:
            for x in range(100):
                ...
                self.levelMap[len(self.levelMap) - 1][rand_y + config.Y_CONST * 3][rand_x + config.X_CONST * 3] = \
                ...
                for y in range(len_room_y):
                    for x in range(len_room_x):
                        if x == 0 or y == 0 or x == len_room_x - 1 or y == len_room_y - 1:
                            self.blankLevel[(y + rand_location_on_map_y) % config.fullLevelMap_Y][
